...
export const R2_STATE_KEY = "clipboard/state.json";
export const R2_UPLOAD_PREFIX = "clipboard/uploads/";
...
export async function listR2UploadKeys() {
  const client = getR2Client();
  const keys = [];
...
export function isUploadStorageKey(value) {
  return typeof value === "string" && value.startsWith(R2_UPLOAD_PREFIX);
}
    ...
    return {
      ...
      items: await Promise.all(
        content.items.map(async (item) => ({
          ...item,
          value: await createSignedReadUrl(item.key),
        })),
      ),
    };
  }
