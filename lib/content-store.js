...
import { deleteR2Object, isR2StorageKey, resolveContentForClient } from "./r2";
...
export async function deleteStoredObjectIfNeeded(previousContent, nextContent) {
  if (
    !["image", "file"].includes(previousContent?.type) ||
  ...
  const isLegacyBlobUrl =
    ...
    /^https?:\/\/.*blob\.vercel-storage\.com\//i.test(previousValue);

  if (previousContent.storage === "r2" || isR2StorageKey(previousValue)) {
    try {
      await deleteR2Object(previousValue);
    } catch {
  ...
  try {
    await del(previousValue, { token: getBlobToken() });
  } catch {
    // Cleanup failure should not block content replacement.
  }
}
